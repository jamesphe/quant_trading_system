...
def main():
    ...
    if args.symbol:
        ...
        try:
            ...
            # 构建stock_pool目录的绝对路径
            stock_pool_dir = os.path.join(current_dir, '..', 'stock_pool')
            # 构建股票列表文件的完整路径
            stock_list_date = args.end_date.replace('-', '')
            stock_list_file = os.path.join(stock_pool_dir, f'stock_list_{stock_list_date}.py')
            
            print(f"尝试导入股票列表文件: {stock_list_file}")
            ...
            if os.path.exists(stock_list_file):
                ...
                print(f"成功读取股票列表，将对 {len(stocks_to_process)} 只股票进行批量优化...")
            else:
                print(f"未找到日期 {args.end_date} 的股票清单，使用默认股票列表...")
                stocks_to_process = STOCK_LIST
                
        except Exception as e:
            print(f"读取股票列表时出错: {str(e)}")
            print("使用默认股票列表...")
            stocks_to_process = STOCK_LIST
            
    if not stocks_to_process:  # 最后的安全检查
        print("无法获取有效的股票列表，程序退出")
        return
