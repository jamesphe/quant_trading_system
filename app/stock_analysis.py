...
def get_stock_analysis_prompt(symbol: str, stock_data: pd.DataFrame, stock_name: str, basic_info: dict, news_list: list, include_backtest: bool = True) -> str:
    ...
    prompt = f"""
你是一名经验丰富的股票量化交易员。你的任务是对{stock_name}（{symbol}）进行深入分析，以“吊灯止损”（Chandelier Stop）策略为核心，对下一交易日的走势进行预判并给出可执行的交易建议。分析策略中必须体现“截止亏损，让利润奔跑”的思想，并需将当前持仓成本纳入考虑范围。
...
【分析要求】  
1. **吊灯止损策略（核心）**：  
   - 确定当前价格相对于吊灯止损位的状态（多头或空头防守区间）。  
   - 使用ATR评估市场波动，为止损位和风险敞口提供参考。  
   - 明确下个交易日的核心策略信号（做多/做空/观望），触及吊灯止损位果断离场。

2. **辅助技术指标验证**（MACD、RSI、BOLL、ZLSMA等）：  
   - 对比吊灯止损信号，确认趋势方向、力度，以及是否存在背离或极端状态（如超买、超卖）。  
   - ZLSMA辅助判断中短期趋势方向，增强策略决策的稳健性。

3. **基本面与市场情绪分析**：  
   - 利用最新资讯、行业动态、龙虎榜数据判断市场情绪。  
   - 若基本面与吊灯止损信号矛盾，请明确说明影响程度和相应调整。

4. **持仓成本纳入策略考量**：  
   - 若当前股价显著高于持仓成本并趋势向上，可考虑上移止损锁定部分利润，让利润奔跑。  
   - 若股价接近或低于持仓成本，且有跌破吊灯止损位风险，应严格执行止损，避免亏损扩大。  
   - 持仓成本与吊灯止损形成上下边界，共同优化进出场时机。

5. **下一交易日走势预判与策略**：  
   - 基于吊灯止损对下个交易日的趋势走向与波动范围进行预判。  
   - 针对不同开盘情况（高开/平开/低开）制定相应行动方案（加仓、减仓、清仓或继续持有），并结合持仓成本判断盈利空间或止损线。

6. **风险控制与仓位管理**：  
   - 严格执行“截止亏损”原则：价格跌破吊灯止损位立即止损离场。  
   - 在趋势向上时，对盈利头寸适当扩大持有时间，随价格走高动态提升止损位，坚守“让利润奔跑”原则。  
   - 结合持仓成本、ATR及吊灯止损位对仓位进行合理分配和调整。

【输出要求】  
- 以吊灯止损策略为中心，结合MACD、RSI、BOLL、ZLSMA等辅助判断，最终给出下个交易日的明晰可执行的交易计划。  
- 明确包含进场/出场点位、止盈止损设置、仓位管理和相应的风险提示。  
- 策略应突出：“触及止损果断离场，趋势有利则让利润奔跑”，并将持仓成本纳入参考，确保收益与风险的动态平衡。
