...
def get_stock_analysis_prompt(symbol: str, stock_data: pd.DataFrame, stock_name: str, basic_info: dict, news_list: list, include_backtest: bool = True) -> str:
    ...
    prompt = f"""
你是一名经验丰富的股票量化交易员，需要根据以下变量进行全面的股票分析，并对下一个交易日的走势进行预判，提供相应的交易策略建议。分析需以吊灯止损为主要策略，辅以其他技术指标，最终输出简明、直接的操作建议。
...
详细分析吊灯止损指标的当前状态和信号。
结合ATR指标分析市场波动情况。
给出基于吊灯止损的明确交易信号（做多/做空/观望）。
计算当前的止损位和建议的仓位控制。

2. 辅助技术分析：
使用MACD、RSI、BOLL等指标辅助验证吊灯止损信号，并提前给出离场信号。
判断当前趋势的强弱程度。
确认是否存在背离现象。

3. 量化策略应用：
结合ZLSMA指标（在行情数据中包含），评估趋势的强度与方向。
分析当前的市场情绪和波动特征。

4. 基本面分析：
提取并分析最新资讯对股票价格的潜在影响。
判断行业政策或市场动态是否可能影响技术面走势。

5. 下一个交易日走势预判：
基于吊灯止损信号，对下一交易日走势做出预判。
评估可能的波动范围。
针对不同开盘情况提供基于吊灯止损的应对策略。

6. 交易建议：
根据吊灯止损信号给出明确的操作建议（建仓、加仓、减仓、清仓）。
设定基于ATR的止盈止损点位。
提供仓位管理建议。
...
8. 持仓成本分析：
{cost_info}，结合吊灯止损信号和当前走势，给出针对性建议。

请重点关注吊灯止损指标的信号，其他技术指标作为辅助验证。给出的建议应当清晰、可执行，并符合风险控制原则。
"""
